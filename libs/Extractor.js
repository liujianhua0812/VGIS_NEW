...
class DataExtractor {
    ...
    BASE_PATH = path.join(__dirname, '../', 'data', 'daily')
    ...
    async initialize () {
        this.workbooks = {}
        let filePattern = /Production Allocation ([\d]+)-([\d]+)-([\d]+)\.xls[xm]?/
        let files = (await fs.readdir(this.BASE_PATH)).filter(file => {
            let dateInfo = file.match(filePattern)
            if (dateInfo) {
                let date = new Date(parseInt(dateInfo[3]), parseInt(dateInfo[2]) - 1, parseInt(dateInfo[1]))
        ...
        for(let i = 0; i < files.length; i++) {
            ...
            let dateInfo = file.match(filePattern)
            let date = new Date(parseInt(dateInfo[3]), parseInt(dateInfo[2]) - 1, parseInt(dateInfo[1]))
            this.workbooks[date.format('yyyy-MM-dd')] = await (new Excel.Workbook().xlsx.readFile(path.join(this.BASE_PATH, file)))
        }
    }
    ...
    async downloadFile (date) {
        for(let key in this.MIME_MAP) {
            try {
                let fileName = `Production Allocation ${date.format('dd-MM-yyyy')}.${key}`
                return {
    ...
    async insertReport (date, file) {
        if (this.TYPE_MAP[file.type]) {
            let fileName = `Production Allocation ${date.format('dd-MM-yyyy')}.${this.TYPE_MAP[file.type]}`
            await fs.copyFile(file.path, path.join(this.BASE_PATH, fileName))
            this.workbooks[date.format('yyyy-MM-dd')] = await (new Excel.Workbook().xlsx.readFile(path.join(this.BASE_PATH, fileName)))
        }
    }
    ...
    async deleteFile (date) {
        ...
        delete this.workbooks[date.format('yyyy-MM-dd')]
    }
