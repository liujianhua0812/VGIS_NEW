...
exports.index = async (ctx, next) => {
    ...
    let Task = global.db.models.malfunction_task
    ...
    if (ctx.request.query.level) {
        query.levelId = ctx.request.query.level
    }
...
exports.levelStat = async (ctx, next) => {
    let Task = global.db.models.malfunction_task
    let AlertLevel = global.db.models.alert_level

    let levels = await AlertLevel.findAll({
        where: {},
        order: [["level", "ASC"]]
    })
    ...
    records = records.reduce((result, item) => {
        result[item.levelId] = parseInt(item.dataValues.total)
        return result
    }, {})

    levels = levels.map(item => Object.assign({}, item.dataValues, { count: records[item.id] || 0 }))
