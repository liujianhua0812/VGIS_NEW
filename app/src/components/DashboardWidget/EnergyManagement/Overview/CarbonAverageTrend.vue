...
import {getFactorValues} from "@/assets/js/api/factor";

export default {
    name: "CarbonAverageTrend",
    components: {
        VgisCard
    },
    computed: {
        trend () {
            ...
            let eFactors = this.factors["电"] || []
            let wFactors = this.factors["水"] || []
            let gFactors = this.factors["天然气"] || []
            let sFactors = this.factors["蒸汽"] || []
            ...
            for(let i = year - 1; i <= year; i++) {
                for(let j = 0; j < 12; j++) {
                    ...
                    let prices = this.sliceFactorsByMonth(eFactors, new Date(i, j))
                    let records = this.sliceRecordsByMonth(eRecords, new Date(i, j))
                    sum += calculateCarbon(prices, records.map(item => ({ value: parseFloat(item.value), time: new Date(item.time) })))

                    prices = this.sliceFactorsByMonth(wFactors, new Date(i, j))
                    records = this.sliceRecordsByMonth(wRecords, new Date(i, j))
                    sum += calculateCarbon(prices, records.map(item => ({ value: parseFloat(item.value), time: new Date(item.time) })))

                    prices = this.sliceFactorsByMonth(gFactors, new Date(i, j))
                    records = this.sliceRecordsByMonth(gRecords, new Date(i, j))
                    sum += calculateCarbon(prices, records.map(item => ({ value: parseFloat(item.value), time: new Date(item.time) })))

                    prices = this.sliceFactorsByMonth(sFactors, new Date(i, j))
        ...
        chartOption () {
            ...
            return {
                tooltip: {
                    ...
                    trigger: "axis",
                    formatter ([params]) {
                        return `<b>${params.name}: </b>${toFixedValue(params.value, 2).toLocaleString()} tCO₂`
                    }
                },
                grid: {
                    right: 24,
    ...
    methods: {
        sliceFactorsByMonth (factors, month) {
            let result = []
            ...
            for(let i = 0; i < factors.length; i++) {
                ...
                if ((!factor.endAt || factor.endAt > start) && (!factor.startAt || factor.startAt <= end)) {
                    result.push(factor)
                }
            }
            return result
