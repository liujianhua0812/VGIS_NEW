<template>
    <vgis-card title="生产数据">
        ...
        >
            ...
            <el-table-column :label="`产量${series.unit ? `（${series.unit.name}）` : ''}`">
                <template slot-scope="{ row }">
                    {{ row.value }}
                </template>
            </el-table-column>
...
import AddEditProduction from "@/components/DashboardWidget/EnergyManagement/ParameterManagement/AddEditProduction.vue";
    ...
    methods: {
        ...
        getSeries () {
            ...
            getTimeSeriesList("StationTop").then(result => {
                this.series = result.data.find(item => item.name === "产量")
            })
        },
        refreshData () {
            getSeriesHistoryValues("总场站", ["产量"], {}).then(result => {
                this.records = result.data["产量"] ? result.data["产量"].values : []
