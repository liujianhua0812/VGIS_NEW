<template>
    <el-dialog class="power-dialog" width="700px" title="故障处理" :visible.sync="dialogVisibility" @closed="close">
        <el-row :gutter="48">
            <el-col :span="11" style="border-right: 1px solid #616161;">
                ...
                <div class="info-label">故障等级</div>
                <div class="info flex align-items-center text-bold">
                    <div :class="['alert', 'm-r-5']" :style="{ background: formData.alert_level.color }">
                        {{formData.alert_level.level}}
                    </div>
                    {{formData.alert_level.name}}
                </div>
                <div class="info-label">内容</div>
                <div class="info">{{record.description}}</div>
                <div class="info-label m-b-10">关联设备</div>
                <div class="info" v-if="task.device_relations.length === 0">暂无</div>
                <el-collapse class="malfunction-reference-list" accordion v-else>
...
import {createTask, getTaskDetail, updateTask} from "@/assets/js/api/malfunction-tasks";
export default {
    name: "HandleMalfunctionDialog",
