    ...
    methods: {
        getSeries () {
            getTimeSeriesList("AirCompressionStation").then(result => {
                this.series = result.data.find(item => item.name === "沿程损失率") || {}
            })
        },
        refreshData () {
            ...
            if (this.station) {
                getSeriesHistoryValues(this.station.id, ["沿程损失率"], {
                    startAt: startDate,
                    endAt: endDate
                }).then(result => {
                    this.records = result.data["沿程损失率"].values
                })
            }
        }
    },
