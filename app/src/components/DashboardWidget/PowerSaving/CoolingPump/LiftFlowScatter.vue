...
export default {
    ...
    methods: {
        getSeries () {
            getTimeSeriesList("CoolingPump").then(result => {
                this.series1 = result.data.find(item => item.name === "扬程") || {}
            })
            getTimeSeriesList("CoolingSystem").then(result => {
        ...
        refreshData () {
            ...
            getNodeParents(this.pump.id).then(result => {
                ...
                let system = parents.find(item => item.model.modelId === "CoolingSystem")
                if (system) {
            ...
            getSeriesHistoryValues(this.pump.id, ["扬程"], {
                startAt: startDate,
                endAt: endDate
            }).then(result => {
                this.records1 = result.data["扬程"].values
            })
        }
    },
    created() {
