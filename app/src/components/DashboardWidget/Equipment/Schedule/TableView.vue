<template>
    <div class="full-h">
        ...
        <el-table
            ...
            <el-table-column label="设备设施" show-overflow-tooltip>
                <template slot-scope="{ row }">
            ...
            <el-table-column label="作业内容" show-overflow-tooltip>
                <template slot-scope="{ row }">
                    {{row.rule_chain.name}}{{ row.rule_chain.description ? `：${row.rule_chain.description}` : "" }}
                </template>
            </el-table-column>
            <el-table-column label="时效" width="120px">
                <template slot-scope="{ row }">
...
import AddEditSchedule from "@/components/DashboardWidget/Equipment/Schedule/AddEditSchedule.vue";
import {
    getScheduleList,
    deleteSchedule,
    bulkDeleteSchedule,
    updateSchedule
} from "@/assets/js/api/automation-schedules";
