<template>
    <el-dialog class="power-dialog" width="700px" :title="`设备运行计划 / ${currentHour}`" :visible.sync="dialogVisibility" @closed="close">
        <el-collapse class="schedule-list" accordion>
            <el-collapse-item v-for="(schedule, index) in timetable" :title="`任务${index + 1} / ${schedule.time.format('yyyy-MM-dd hh:mm:ss')}`">
                <div class="p-10">
                    ...
                    <div class="info">
                        {{ schedule.info.device_relations.map(item => `${item.model_instance.name}（${item.model_instance.instanceId}）`).join(", ") }}
                    </div>
                    <div class="info-label">作业内容</div>
                    <div class="info">{{schedule.info.rule_chain.name}}{{schedule.info.rule_chain.description ? `：${schedule.info.rule_chain.description}` : ""}}</div>
                </div>
            </el-collapse-item>
        </el-collapse>
    </el-dialog>
