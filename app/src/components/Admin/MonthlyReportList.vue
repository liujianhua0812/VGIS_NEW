      ...
      <el-table border header-cell-class-name="table-header-cell" cell-class-name="table-cell" :data="calendarData">
        ...
        <el-table-column v-for="(month, index) in months" :key="month" :label="month">
          <template slot-scope="{ row }">
            ...
            <el-popover v-auth="{ resources: 'Monthly Report', action: 'Admin' }" trigger="click" width="300" :ref="`form-${new Date(row.year, index, 1).format('yyyy-MM')}`">
...
<script>
  ...
  import {
    ...
    uploadMonthlyReport,
    removeMonthlyReport
  } from "../../assets/js/api/monthly-report";
  ...
  export default {
    ...
    data() {
      return {
        ...
        navs: [{
          ...
          name: this.$t("menu.monthlyReport")
        }],
        filter: {
          startAt: '',
          endAt: ''
    ...
    methods: {
      ...
      upload(date) {
        ...
        uploadMonthlyReport(this.formData).then(result => {
          this.$message({
            type: 'success',
      ...
      remove(date) {
        this.$confirm(this.$t("message.shared.confirm"), this.$t("message.shared.warning"), {
          ...
          removeMonthlyReport(date).then(result => {
            this.$message({
              type: 'success',
      ...
      refresh() {
        getMonthlyReports(this.filter).then(result => {
