      ...
      <el-table border header-cell-class-name="table-header-cell" cell-class-name="table-cell" :data="calendarData">
        ...
        <el-table-column v-for="(month, index) in months" :key="month" :label="month">
          <template slot-scope="{ row }">
            ...
            <el-popover v-auth="{ resources: 'Environment Report', action: 'Admin' }" trigger="click" width="300" :ref="`form-${new Date(row.year, index, 1).format('yyyy-MM')}`">
...
<script>
  ...
  import {
    ...
    getEnvironmentReports,
    uploadEnvironmentReport,
    removeEnvironmentReport
  } from "../../assets/js/api/environment-report";

  export default {
    name: "EnvironmentReportList",
    components: {
      VgisBreadcrumb
    },
    data() {
      return {
        ...
        navs: [{
          ...
          name: this.$t("menu.environmentReport")
        }],
        filter: {
          startAt: '',
          endAt: ''
    ...
    methods: {
      selectReport(file) {
        if (!['PDF'].includes(file.name.substr(file.name.length - 3, 3).toUpperCase())) {
          this.$message({
            type: 'error',
            message: this.$t("form.report.report.error.pdfOnly"),
            showClose: true
          })
        } else {
      ...
      upload(date) {
        ...
        uploadEnvironmentReport(this.formData).then(result => {
          this.$message({
            type: 'success',
      ...
      remove(date) {
        this.$confirm(this.$t("message.shared.confirm"), this.$t("message.shared.warning"), {
          ...
          removeEnvironmentReport(date).then(result => {
            this.$message({
              type: 'success',
      ...
      refresh() {
        getEnvironmentReports(this.filter).then(result => {
