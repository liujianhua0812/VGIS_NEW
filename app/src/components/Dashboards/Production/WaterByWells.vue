      ...
      getNodesByModel("Well").then(result => {
        ...
        getNodeDetail(wellNames[0]).then(nodeDetails => {
          ...
          getMultipleSeriesHistoryValues(
            ...
            this.wells = []
            wellNames.map(name => {
              let wellItemWater = res.data[name]['Daily Water Production']
              // let wellItemOil = res.data[name]['Daily Oil Production']
              if (wellItemWater.values.length >= 2) {
                ...
                this.chartData.push({
                  name: name,
                  today: this.convertData(this.seriesData["Daily Water Production"],todayWater ),
                  // today: todayWater + todayOil,
                  yesterday: this.convertData(this.seriesData["Daily Water Production"],  yesterdayWater),
                  // yesterday:  yesterdayOil + yesterdayWater,
                  diff: Math.abs(todayWater -  yesterdayWater)
                })
              }
            })
